...
directory = '/home/weilan/Desktop/Raf_shotgun_desktop/Raf_shotgun_dbCAN3_contigs/Contigs_PUL_Substrates/fam_substrate_abund'
...
for file_name in file_names:
    ...
    # Read the file
    data = pd.read_csv(file_path, sep="\t", usecols=["Substrate", "Abundance"])
    # Correctly extract the sample ID from the filename
    sample_id = file_name.split("_fam_substrate_abund.out")[0]  # Gets the part before '_fam_substrate_abund.out'
...
adjusted_output_path = os.path.join(output_directory, "Combined_fam_substrate_abund.csv")
