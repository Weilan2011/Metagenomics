...
directory = '/home/weilan/Desktop/Raf_shotgun_desktop/Raf_shotgun_dbCAN3_contigs/Contigs_PUL_Substrates/subfam_abund'
...
for file_name in file_names:
    ...
    # Read the file
    data = pd.read_csv(file_path, sep="\t", usecols=["Subfamily", "Abundance"])
    # Correctly extract the sample ID from the filename
    sample_id = file_name.split("_subfam_abund.out")[0]  # Gets the part before '_fam_abund.out'
    data.rename(columns={"Abundance": sample_id}, inplace=True)
    # Merge the data into the adjusted DataFrame
    if adjusted_data.empty:
        adjusted_data = data.set_index("Subfamily")
    else:
        adjusted_data = adjusted_data.join(data.set_index("Subfamily"), how="outer")
...
adjusted_output_path = os.path.join(output_directory, "Combined_subfam_abund.csv")
